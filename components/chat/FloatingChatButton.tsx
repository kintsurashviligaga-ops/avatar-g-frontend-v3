...
export function FloatingChatButton({ hidden = false }: { hidden?: boolean }) {
  ...
  // Hide while the soft keyboard is open. These are position:fixed at the bottom,
  // so on a non-immersive page with a bottom composer (e.g. /chat) they float over
  // the composer's corner controls (mic/send/attach) and swallow taps. The keyboard
  // being open is exactly when the user is interacting with that composer.
  ...
  return (
    <>
      ...
      <button
        ...
        type="button"
        className="floating-call-btn floating-btn-bottom fixed left-4 z-[9999] flex items-center justify-center w-14 h-14 rounded-full transition-all duration-300 hover:scale-105 hover:-translate-y-0.5 active:scale-95 pointer-events-auto"
        style={{
      ...
      <Link
        ...
        className="floating-chat-btn floating-btn-bottom fixed right-4 z-[9999] flex items-center justify-center w-14 h-14 rounded-full transition-all duration-300 hover:scale-105 hover:-translate-y-0.5 active:scale-95 pointer-events-auto"
        style={{
