  ...
  return (
    <div className="w-full">
      ...
      <div className="mt-2.5 space-y-1">
        <div onClick={seek} className="group relative h-2 cursor-pointer rounded-full bg-app-border/25" role="slider" aria-valuemin={0} aria-valuenow={Math.round(cur)} aria-valuemax={Math.round(dur)} tabIndex={0}>
          <div className="absolute inset-y-0 left-0 rounded-full bg-cyan-400" style={{ width: `${pct}%` }} />
          <div className="absolute top-1/2 h-3.5 w-3.5 -translate-y-1/2 rounded-full bg-white opacity-0 shadow transition-opacity group-hover:opacity-100" style={{ left: `calc(${pct}% - 7px)` }} />
        </div>
