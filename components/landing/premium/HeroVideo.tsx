  ...
  return (
    ...
    >
      <div className="relative w-full" style={{ aspectRatio: '16 / 9' }}>
        ...
        {/* CinematicPoster — always rendered as underlay, fades out when video is ready */}
        <div
          className="absolute inset-0 z-[2] transition-opacity duration-700"
          style={{ opacity: videoReady && !videoError ? 0 : 1, pointerEvents: videoReady ? 'none' : 'auto' }}
        >
          <CinematicPoster onPlayClick={togglePlay} />
        </div>

        {/* Video element — always rendered, fades in when ready */}
        <video
          ref={videoRef}
          className="absolute inset-0 w-full h-full object-cover rounded-2xl z-[3] transition-opacity duration-700"
        ...
        <div
          ...
          style={{
            opacity: videoReady && controlsVisible ? 1 : 0,
            transform: videoReady && controlsVisible ? 'translateY(0)' : 'translateY(8px)',
            pointerEvents: videoReady && controlsVisible ? 'auto' : 'none',
        ...
        {!playing && (
          <div className="absolute inset-0 z-[25] flex items-center justify-center pointer-events-none">
            <div
              className="w-16 h-16 sm:w-20 sm:h-20 rounded-full flex items-center justify-center transition-opacity duration-300"
              style={{
...
function CinematicPoster({ onPlayClick }: { onPlayClick?: () => void }) {
  return (
    <div
      className="absolute inset-0 rounded-2xl flex flex-col items-center justify-center gap-6 cursor-pointer"
      onClick={onPlayClick}
      style={{
