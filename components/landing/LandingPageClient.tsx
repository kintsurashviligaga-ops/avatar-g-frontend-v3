...
const WorkflowBuilder = dynamic(() => import("@/components/landing/WorkflowBuilder").then((m) => m.WorkflowBuilder), {
  ssr: false,
  loading: () => <SectionFallback label="Workflow Builder" />,
});
...
export default function LandingPageClient() {
  ...
  // ── ORBIT COPY (kept for orbit section below) ──
  const orbitCopy = {
    en: {
  ...
  const currentOrbit = orbitCopy[locale as keyof typeof orbitCopy] ?? orbitCopy.en;
  ...
  return (
    <div className="relative min-h-screen bg-transparent text-white overflow-x-hidden">
      ...
      <div className="relative pointer-events-auto" style={{ zIndex: 10 }}>
        ...
        <LandingSection>
          <ErrorBoundary fallback={<SectionFallback label="Workflow Builder" />}>
            <WorkflowBuilder />
          </ErrorBoundary>
        </LandingSection>
