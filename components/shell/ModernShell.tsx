...
function IconLogout() {
  return <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M9 21H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h4"/><polyline points="16 17 21 12 16 7"/><line x1="21" y1="12" x2="9" y2="12"/></svg>
}
function IconGlobe() {
...
/* ─── Service search data ─── */
const SEARCHABLE = [
  { slug: 'avatar', label: { en: 'AI Avatar', ka: 'AI ავატარი', ru: 'AI Аватар' } },
  { slug: 'video', label: { en: 'Video Studio', ka: 'ვიდეო სტუდია', ru: 'Видео-студия' } },
  { slug: 'image', label: { en: 'Image Creator', ka: 'სურათის შემქმნელი', ru: 'Изображения' } },
  { slug: 'music', label: { en: 'Music Studio', ka: 'მუსიკის სტუდია', ru: 'Музыка' } },
  { slug: 'photo', label: { en: 'Photo Studio', ka: 'ფოტო სტუდია', ru: 'Фото' } },
  { slug: 'editing', label: { en: 'Video Editing', ka: 'ვიდეო რედაქტირება', ru: 'Монтаж' } },
  { slug: 'media', label: { en: 'Media Production', ka: 'მედია', ru: 'Медиа' } },
  { slug: 'text', label: { en: 'Text Intelligence', ka: 'ტექსტი', ru: 'Текст AI' } },
  { slug: 'prompt', label: { en: 'Prompt Builder', ka: 'პრომფტი', ru: 'Промпт' } },
  { slug: 'visual-intel', label: { en: 'Visual Intelligence', ka: 'ვიზუალური AI', ru: 'Визуальный AI' } },
  { slug: 'workflow', label: { en: 'Workflows', ka: 'ავტომატიზაცია', ru: 'Автоматизация' } },
  { slug: 'agent-g', label: { en: 'Agent G', ka: 'Agent G', ru: 'Агент G' } },
  { slug: 'business', label: { en: 'Business AI', ka: 'ბიზნეს AI', ru: 'Бизнес AI' } },
  { slug: 'shop', label: { en: 'Online Shop', ka: 'მაღაზია', ru: 'Магазин' } },
  { slug: 'software', label: { en: 'Software Dev', ka: 'პროგრამირება', ru: 'Разработка' } },
  { slug: 'tourism', label: { en: 'Tourism AI', ka: 'ტურიზმი', ru: 'Туризм' } },
] as const

/* ─── Sidebar links ─── */
const SIDEBAR_NAV = [
  { path: '/services', icon: '◆', label: { en: 'All Services', ka: 'სერვისები', ru: 'Все сервисы' } },
  { path: '/services/avatar', icon: '✦', label: { en: 'Avatar', ka: 'ავატარი', ru: 'Аватар' } },
  { path: '/services/video', icon: '▶', label: { en: 'Video', ka: 'ვიდეო', ru: 'Видео' } },
  { path: '/services/image', icon: '◆', label: { en: 'Image', ka: 'სურათი', ru: 'Изображение' } },
  { path: '/services/music', icon: '♪', label: { en: 'Music', ka: 'მუსიკა', ru: 'Музыка' } },
  { path: '/business', icon: '◎', label: { en: 'Business', ka: 'ბიზნესი', ru: 'Бизнес' } },
  { path: '/pricing', icon: '⊞', label: { en: 'Pricing', ka: 'ფასები', ru: 'Тарифы' } },
] as const
...
export function TopNavbar({ onMenuToggle, menuOpen }: { onMenuToggle: () => void; menuOpen: boolean }) {
  ...
  const filtered = searchQuery.trim()
    ? SEARCHABLE.filter(s => {
        ...
        return s.slug.includes(q) || Object.values(s.label).some(l => l.toLowerCase().includes(q))
      })
    : []
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3">
        ...
        <Link href={lh('/')} className="flex items-center gap-2 hover:opacity-80 transition-opacity">
          <span className="font-bold text-[1.05rem] tracking-tight" style={{ color: 'var(--color-text)' }}>
            MyAvatar<span style={{ color: 'var(--color-accent)', opacity: 0.7 }}>.ge</span>
          </span>
        </Link>
      </div>
        ...
        {searchOpen && filtered.length > 0 && (
          ...
          >
            {filtered.slice(0, 6).map(s => (
              ...
              >
                <span style={{ color: 'var(--color-text-tertiary)' }}><IconGrid /></span>
...
export function SidebarMenu({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <>
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 h-16 shrink-0" style={{ borderBottom: '1px solid var(--color-border)' }}>
          ...
          {SIDEBAR_NAV.map(item => (
            <Link
              ...
              style={{
                ...
                backgroundColor: isActive(item.path) ? 'var(--color-accent-soft)' : 'transparent',
              }}
            >
