...
export function DashboardSidebar({ userRole = 'seller', className }: SidebarProps) {
  ...
  return (
    <aside className={cn(
      'w-64 bg-[linear-gradient(180deg,rgba(3,7,18,0.97),rgba(5,10,25,0.95))] border-r border-white/[0.07] min-h-screen p-6 backdrop-blur-2xl',
      className
    )}>
      {/* Header */}
      <div className="mb-8">
        ...
        {filteredItems.map((item) => (
          ...
          >
            <div className="flex items-center gap-3">
              <div className={cn(
                'transition-colors',
                isActive(item.href) ? 'text-cyan-400' : 'text-white/30 group-hover:text-white/70'
              )}>
                {item.icon}
              </div>
