...
const NAV_ITEMS: DashboardSidebarItem[] = [
  { key: 'dashboard', icon: '◉', href: '/dashboard', exact: true },
  { key: 'create', icon: '✦', href: '/services' },
  { key: 'agentG', icon: '◈', href: '/services/agent-g', primary: true },
  { key: 'workflows', icon: '◎', href: '/services/workflow' },
  { key: 'analytics', icon: '▣', href: '/dashboard' },
  { divider: true },
  { key: 'billing', icon: '◻', href: '/account/billing' },
  { key: 'settings', icon: '⚙', href: '/account/business' },
]

export function DashboardSidebar({ locale = 'en', collapsed = false, onToggle, brandLabel = 'MyAvatar', navItems = NAV_ITEMS, showToggle = true }: DashboardSidebarProps) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 px-4 h-14 shrink-0" style={{ borderBottom: '1px solid var(--nav-border)' }}>
        ...
        {!collapsed && <span className="text-sm font-semibold" style={{ color: 'var(--color-text)' }}>{brandLabel}</span>}
        {showToggle && onToggle && (
          <button
            onClick={onToggle}
        ...
        {navItems.map((item, i) => {
          ...
          return (
            <Link
              ...
              title={collapsed ? (item.label ?? t(`sidebar.${item.key}`)) : undefined}
            >
              <span
