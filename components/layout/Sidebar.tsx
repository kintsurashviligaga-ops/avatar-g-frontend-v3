...
// Locale-keyed nav: the app routes under /{locale}/… (next-intl middleware), so a bare
...
const coreItems: Array<{ href: string; icon: typeof LayoutGrid; labels: Record<Loc, string> }> = [
  ...
  { href: '/shop', icon: ShieldCheck, labels: { ka: 'მაღაზია', en: 'Shop', ru: 'Магазин' } },
];

export function Sidebar() {
  const pathname = usePathname();
  const seg = pathname.split('/')[1];
  const locale: Loc = seg === 'en' ? 'en' : seg === 'ru' ? 'ru' : 'ka';
