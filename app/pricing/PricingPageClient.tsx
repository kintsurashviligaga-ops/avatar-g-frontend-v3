      ...
      <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', gap: 9, flex: 1 }}>
        {plan.featuresKa.map((f, i) => (
          <li key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 9, fontSize: 13, color: 'rgba(255,255,255,0.65)', lineHeight: 1.4 }}>
            <Check style={{ width: 14, height: 14, color: isPopular ? '#f59e0b' : '#0ea5e9', flexShrink: 0, marginTop: 1 }} />
            {f}
          </li>
        ))}
      </ul>
    ...
    <div style={{
      ...
      <header style={{
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <Link href="/signup" style={{ padding: '7px 16px', borderRadius: 8, background: 'linear-gradient(135deg, #0369a1, #0ea5e9)', color: '#fff', textDecoration: 'none', fontSize: 13, fontWeight: 600 }}>
            დარეგისტრირება
          </Link>
        </div>
      </header>
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: 1100, margin: '0 auto', padding: '60px 20px 80px' }}>
        ...
        >
          ...
          <h1 style={{ fontSize: 'clamp(28px, 5vw, 48px)', fontWeight: 900, color: '#fff', margin: '0 0 16px', lineHeight: 1.15 }}>
            გეგმა{' '}
            <span style={{ background: 'linear-gradient(135deg, #0ea5e9, #f59e0b)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
                ...
                {FEATURE_ROWS.map((row, i) => (
                  <tr key={i} style={{ background: i % 2 === 0 ? 'rgba(255,255,255,0.015)' : 'transparent' }}>
                    ...
                    {row.values.map((val, j) => (
                      <td key={j} style={{ padding: '11px 16px', textAlign: 'center', fontSize: 12, fontWeight: val === '✓' ? 700 : 500, color: val === '—' ? 'rgba(255,255,255,0.2)' : val === '✓' ? '#0ea5e9' : 'rgba(255,255,255,0.75)', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                        {val}
                      </td>
                    ))}
                  </tr>
        ...
        >
          <p style={{ fontSize: 14, color: 'rgba(255,255,255,0.6)', margin: 0, lineHeight: 1.7 }}>
            ...
            <br />
            კითხვები? <a href="mailto:support@myavatar.ge" style={{ color: '#0ea5e9', textDecoration: 'none', fontWeight: 600 }}>support@myavatar.ge</a>
          </p>
        </motion.div>
      </div>
    </div>
