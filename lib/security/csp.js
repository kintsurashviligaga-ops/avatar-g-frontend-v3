...
function cspDirectiveMap() {
  return {
    ...
    'connect-src': [
      ...
      // LiveAvatar real-time avatar (LiveKit): the browser opens a wss signaling socket to the LiveKit
      // room the /api/avatar/live/session mint returns. WebRTC MEDIA rides over UDP/TCP (not connect-src),
      // and the avatar's video/audio tracks attach via srcObject (which bypasses media-src) — only the
      // signaling wss + the LiveAvatar REST base need allowing. livekit.cloud is the common backend;
      // liveavatar.com covers a proxied/self-hosted room. Inert until a session is actually minted.
